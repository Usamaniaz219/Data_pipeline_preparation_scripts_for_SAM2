...
image_dir = '/media/usama/SSD/Data_for_SAM2_model_Finetuning/Training_data_for_sam2_/triple prompt 15 cities/images/'
mask_dir = '/media/usama/SSD/Data_for_SAM2_model_Finetuning/Training_data_for_sam2_/triple prompt 15 cities/masks/'
txt_dir = '/media/usama/SSD/Data_for_SAM2_model_Finetuning/Training_data_for_sam2_/triple prompt 15 cities/new_txt_files/'
background_txt_dir = "/media/usama/SSD/Data_for_SAM2_model_Finetuning/Training_data_for_sam2_/triple prompt 15 cities/background_txt_files/"
...
def load_image_mask_and_forground_background_points(image_dir, mask_dir, txt_dir, background_txt_dir):
    ...
    for image_filename in image_files:
        ...
        txt_filename = os.path.join(txt_dir, os.path.splitext(image_filename)[0] + '.txt')  # Foreground points
        background_txt_filename = os.path.join(background_txt_dir, os.path.splitext(image_filename)[0] + '.txt')  # Background points
        ...
        # Read foreground points from the text file
        with open(txt_filename, 'r') as f:
            points = [list(map(int, line.strip().split(','))) for line in f.readlines()]
            # print("Foreground points:", points)

        # Read background points from the text file
        with open(background_txt_filename, 'r') as f:
            background_points = [list(map(int, line.strip().split(','))) for line in f.readlines()]
            # print("Background points:", background_points)

        # Resize image and mask
        scaling_factor = np.min([1024 / Img.shape[1], 1024 / Img.shape[0]])  # Scaling factor
        Img = cv2.resize(Img, (int(Img.shape[1] * scaling_factor), int(Img.shape[0] * scaling_factor)))
        ann_map = cv2.resize(ann_map, (int(ann_map.shape[1] * scaling_factor), int(ann_map.shape[0] * scaling_factor)))
        ...
        # Yield the processed image, mask, combined points, and labels
        yield Img, np.array(ann_map), all_points, all_labels
...
FINE_TUNED_MODEL_NAME = "fine_tuned_sam2_16_december_2024_16_accumulations_step"
...
for epoch in range(1, no_of_epochs + 1):
    data_generator_21 = load_image_mask_and_forground_background_points(image_dir, mask_dir, txt_dir,background_txt_dir)
    for idx , (image, mask, input_point, input_label) in enumerate(data_generator_21):
        ...
        print("input_labels",input_label)
        ...
        seg_loss = F.binary_cross_entropy(prd_mask, gt_mask)
        # seg_loss = FocalLoss(gamma=2)(prd_mask,gt_mask)
